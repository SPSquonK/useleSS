...
BOOL RunPredefineKey( LPCTSTR szKey, CNpcDialogInfo& info )
{
	...
	{
	case QE_QUEST_END:
		__QuestEnd( info.GetPcId(), info.GetNpcId(), nGlobal, QuestId::From(info.GetQuest()), FALSE );
		break;
	case QE_QUEST_BEGIN:
		__QuestBegin( info.GetPcId(), info.GetNpcId(), QuestId::From(info.GetQuest()) );
		break;
	case QE_QUEST_BEGIN_YES:
		__QuestBeginYes( info.GetPcId(), info.GetNpcId(), QuestId::From(info.GetQuest()) );
		break;
	case QE_QUEST_BEGIN_NO:
		__QuestBeginNo( info.GetPcId(), info.GetNpcId(), QuestId::From(info.GetQuest()) );
		break;
	case QE_QUEST_END_COMPLETE:
		__QuestEndComplete( info.GetPcId(), info.GetNpcId(), nGlobal, info.GetReturn(), QuestId::From(info.GetQuest()) );
		break;
	case QE_QUEST_END_FAIL:
		__QuestEnd( info.GetPcId(), info.GetNpcId(), nGlobal, QuestId::From(info.GetQuest()), TRUE );
		break;
	default:
		return FALSE;
	};
	...
	if( pQuestProp )
	{
		...
		if( pQuestProp->m_nEndRewardPKValueMin || pQuestProp->m_nEndRewardPKValueMax )
		{
			if( pQuestProp->m_nEndRewardPKValueMin <= pQuestProp->m_nEndRewardPKValueMax )
			{
				...
				pUser->AddPKValue();
				g_dpDBClient.SendLogPkPvp( pUser, NULL, 0, 'P' );
		...
		for( int i = 0; i < MAX_QUESTREMOVE; i++ )
		{
			...
			{
				__RemoveQuest( nPcId, QuestId::From(pQuestProp->m_anEndRemoveQuest[ i ]) );
			}
		}
		// 퀘스트 종료 
	...
	if( bCompleteCheck )
	{
		if (pUser->m_quests) {
			for (QUEST & quest : pUser->m_quests->current) {
				...
				if (!pQuestProp) {
					WriteError("__QuestEnd quest(%d) property null", quest.m_wId.get());
					continue;
				}
...
void __QuestBeginYes( int nPcId, int nNpcId, QuestId nQuestId )
{
	...
	if( __IsBeginQuestCondition( pUser, nQuestId )
		&& lpQuest == NULL
		&& !pUser->IsCompleteQuest( nQuestId ) )
	{
