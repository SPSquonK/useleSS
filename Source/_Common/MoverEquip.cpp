	...
	EQUIP_INFO * pEquipInfo, CModelObject* pModel, BOOL bEquip, CMover *pMover )
{
	...
	if( !bEquip )
	{
		if( dwParts == PARTS_RING1 || dwParts == PARTS_EARRING1 )
		{
			if( pInventory )
			{
				CItemElem * pItemElemTemp = pInventory->GetEquip( dwParts );
	...
	if( bEquip )
	{
		...
		else
		{
			...
			if( dwParts == PARTS_RING1 || dwParts == PARTS_EARRING1 )
			{
				...
				CItemElem* pItemElemTemp1 = pInventory->GetEquip( dwParts );
				CItemElem* pItemElemTemp2 = pInventory->GetEquip( dwParts + 1 );
...
void CMover::SetDestParamEquip( const ItemProp* pItemProp, CItemElem* pItemElem, BOOL bIgnoreSetItem )
{
	...
	if( pItemElem && pItemElem->m_nResistAbilityOption && pItemElem->GetProp()->dwItemKind1 == IK1_ARMOR )
	{
		...
		{
			const SAI79::ePropType eProp = pItemElem->m_bItemResist;
