...
void CUserTaskBar::Serialize( CAr &ar )
{
	if( ar.IsStoring() )
	{
		...
		for( int i = 0; i < MAX_SLOT_APPLET; i++ )
		{
			if( m_aSlotApplet[i].m_dwShortcut != ShortcutType::None)		// 값이 들어있는지 검사
			{
		...
		*(UNALIGNED int*)( lpBuf + uOffset )	= nCount;
		...
		for( int i = 0; i < MAX_SLOT_ITEM_COUNT; i++ )
		{
			for( int j = 0; j < MAX_SLOT_ITEM; j++ )
			{
				if( m_aSlotItem[i][j].m_dwShortcut != ShortcutType::None)		// 값이 들어있는지 검사
				{
					ar << i << j;
					ar << m_aSlotItem[i][j];
					nCount++;
		...
		*(UNALIGNED int*)( lpBuf + uOffset )	= nCount;
		...
		{
			if( m_aSlotQueue[i].m_dwShortcut != ShortcutType::None)		// 값이 들어있는지 검사
			{
				ar << i;
				ar << m_aSlotQueue[i];
				nCount++;
