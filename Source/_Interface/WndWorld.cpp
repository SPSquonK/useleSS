...
void CCaption::Render( CPoint ptBegin, C2DRender* p2DRender )
{
	...
	for (CAPTION & lpCaption : m_aCaption) {
		...
		const FLOAT fScale = ( 7.0f - lpCaption.m_fScale * 6.0f ); // ÃÖ´ë 7¹è »çÀÌÁî 
...
/*
	...
	if( g_pPlayer )
	{
		...
		_stprintf( strDebug, _T("½Ã·Ã¸ðµå %d" ), g_pPlayer->IsMode(RECOVERCHAO_MODE)  );
...
*/			
	...
	if( pItemElem &&  pItemElem->GetProp()->dwItemKind3 == IK3_WAND )
	{
		...
		if( pFocus && pFocus->GetType() == OT_MOVER && g_pPlayer->IsLive() )
		{
			...
			if( m_bLButtonDown && m_bSelectTarget )
			{
				...
				{	// ¸¶³ª°¡ ¸ðÀÚ¸¥»óÈ²ÀÌ¸é ÃÖ°í ´Ü°è±îÁö ¿Ã¶ó°¡Áö ¾Ê°í ÃÖ°í -1´Ü°è Â¥¸®¸¦ ½ð´Ù. 
	...
	if( bRenderGauge )
	{
		if( nPower == -1 )	// ÃÊ°í´Ü°è °ø°Ý ÈÄ µô·¹ÀÌ »óÅÂ?
	...
	// ³¯±â ¸ðµå°¡ ¾Æ´Ï¸é °íµµ°è Ãâ·Â ¾ÈÇÏ°í Á¾·á 
...
#ifdef __CLIENT
	// ¸Ó¸®À§¿¡ ¶ß´Â µ¥¹ÌÁö ¼ýÀÚ¸¦ À§ÇØ °ÔÀÓÈ­¸é ºäÇ¥Æ®¸¦ ¹Þ¾ÆµÒ.
	...
	// ºñÇàÁß »ç°æ¹Ý°æÀÌ³»¿¡ µé¾î¿À´Â ÇÃ·¹ÀÌ¾îµéÀº »ç°¢Å×µÎ¸®°¡ ÃÄÁø´Ù.
	{
		for (const OBJID objid : m_flyTarget.GetAll()) {
			CMover * pMover = prj.GetMover(objid);
			if (!IsValidObj(pMover)) continue;
			
			CRect rect = GetBoundRect( pMover );		// È­¸é»ó¿¡¼­ÀÇ ¹Ù¿îµå ·ºÆ®¸¦ ±¸ÇÔ.
...
/* chipi_090814 - ÇÎ Ãâ·ÂÀ» µð¹ö±× Ã¢À¸·Î ÀÌµ¿
	// ÇÎ(Ping) Ãâ·Â
...
void CWndWorld::RenderFocusArrow( CPoint pt )
{
	...
	if( bAdjust )
	{
		...
		if( (int)ptOut.x > (int)g_Neuz.GetCreationWidth() )	// ¿À¸¥ÂÊÀ¸·Î ¹þ¾î³².
		{
			bAdjust2 = TRUE;		// È­¸éÀ» ¹þ¾î³µÀ¸¹Ç·Î È­»ìÇ¥·Î Ç¥½ÃÇÔ.
			ptOut.x = (LONG)g_Neuz.GetCreationWidth() - 32;		// °æ°èÁÂÇ¥¸¦ ¼³Á¤.
		}
		if( (int)ptOut.x < 0 )	// ¿ÞÂÊÀ¸·Î ¹þ¾î³µÀ»¶§.
		{
			bAdjust2 = TRUE;		// È­¸éÀ» ¹þ¾î³µÀ¸¹Ç·Î È­»ìÇ¥·Î Ç¥½ÃÇÔ.
			ptOut.x = 32;
		}
		...
		if( (int)rectBound.top < 0 )		// À§·Î ¹þ¾î³µÀ»¶§.
		{
			bAdjust2 = TRUE;		// È­¸éÀ» ¹þ¾î³µÀ¸¹Ç·Î È­»ìÇ¥·Î Ç¥½ÃÇÔ.
			ptOut.y = 32;
		}
	...
	for (CLandscape * pLand : pWorld->GetVisibleLands())
	{
		for (CObj * pObj : pLand->m_apObjects[OT_MOVER].ValidObjs()) {
			...
			if( g_WndMng.m_nObjectFilter == OBJFILTER_NPC && ( pMover->IsPlayer() == TRUE || pMover->IsPeaceful() ) )
				continue;
				*/
			if( pMover->IsMode( TRANSPARENT_MODE ) )	// ´ë»óÀÌ Åõ¸í¸ðµåÀÏ¶© Å¸°Ù ¾ÈµÊ.			
				continue;	
			DWORD dwColor = 0xffffffff;
			if( pObj == g_pPlayer )
				dwColor = 0xffffff00;
...
void CWndWorld::OnInitialUpdate()
{
	...
	m_texTargetFly.LoadScript( MakePath( DIR_ICON, "icon_FlightTargetB.inc" ) );			// ºñÇà¸ðµå½Ã Å¸°Ù 4±ÍÅüÀÌ.		sun!!
	m_texTargetArrow.LoadScript( MakePath( DIR_ICON, "icon_FlightTargetArrow.inc" ) );	// ºñÇà¸ðµå½Ã Å¸°Ù¹æÇâÀ» °¡¸£Å°´Â È­»ìÇ¥
	m_texTargetArrow.GetAt(0)->m_ptCenter.x += 32;
	m_texTargetArrow.GetAt(0)->m_ptCenter.y += 32;
	m_texGauFlight.LoadScript( MakePath( DIR_THEME, "Theme_GauFlight.inc" ) );		// ºñÇà¸ðµå½Ã °ÔÀÌÁö ÀÎÅÍÆäÀÌ½º
	m_texFontDigital.LoadScript( MakePath( DIR_THEME, "Theme_FontDigital1.inc" ) );		// µðÁöÅ»¸ð¾çÀÇ ÆùÆ®.
	...
	if( g_pPlayer->IsMode( MODE_MAILBOX ) )
	{
		...
		if( pWndInstantMsg )
		{
			...
			pWndInstantMsg->m_timer.Set( MIN(1) );		//chipi_080808 - ¿ìÆí ¸Þ¼¼Áö Ã¢ À¯Áö 1ºÐÀ¸·Î º¯°æ
		}
...
BOOL CWndWorld::OnCommand( UINT nID, DWORD dwMessage, CWndBase* pWndBase )
{
		...
		case MMI_POST:
			{
				...
				if( pWndBaseBuf )
				{
					//g_WndMng.PutString( "ÀÌ¹Ì ¸ÞÀÌÅ©¾÷À» ÀÌ¿ëÁßÀÔ´Ï´Ù", NULL, 0xffffff00 );
	...
	case MMI_RENAME_CANCEL:
		...
		case MMI_DUEL:
			if( pFocusMover->IsPlayer() )
			{
				if( pFocusMover->IsDie() )
					break;
				g_DPlay.SendDuelRequest( g_pPlayer, pFocusMover );		// »ó´ë¿¡°Ô ¸ÂÂ¯¶ßÀÚ°í ¸Þ½ÃÁö ³¯¸².
			}
			break;
		case MMI_DUEL_PARTY:
			if( pFocusMover->IsPlayer() )
			{
				if( pFocusMover->IsDie() )
					break;
				g_DPlay.SendDuelPartyRequest( g_pPlayer, pFocusMover );		// »ó´ë¿¡°Ô ÇÑÆÇ¶ßÀÚ°í ¸Þ½ÃÁö ³¯¸².
			}
			break;
		case MMI_TRACE:
		...
		case MMI_INPUT_REWARD: // Çö»ó±Ý °É±â
			{
			// Çö»ó¹ü ¸®½ºÆ® ¿äÃ» ÆÐÅ¶
		...
		case MMI_GUILDHOUSE_CHARGE:	//À¯Áöºñ 
			{
				...
				if( !g_WndMng.m_pWndUpkeep )
				{
					...
					g_WndMng.m_pWndUpkeep->Initialize();	//gmpbigsun( 2010_05_12 ) : Á¾·á¹ö±×¼öÁ¤ , ÀÌ³à¼®ÀÇ ºÎ¸ð°¡ ¿ùµåmng¿©¼­ Á¢¼ÓÁ¾·áÇÏ´Ù »¸À½.
...
BOOL CWndWorld::UseFocusObj( CCtrl* pFocusObj )
{
	...
	if( g_pPlayer->m_dwMode & DONMOVE_MODE )	return FALSE;	// µ·¹«ºê ¸ðµå¸é ¾Ï°Íµµ ¸øÇÔ.
...
//	DEBUG_CHATMSG( "Å¸°Ù»ç¿ë", NULL, 0xffffffff );
	...
	else if( pFocusObj->GetType() == OT_MOVER )
	{
		...
		// ¿©±â´Â ¹«Á¶°Ç °ø°ÝÀÎµ¥.. ´Ü°Å¸® °ø°ÝÀº ¸ñÇ¥¸¸ ¼¼ÆÃÇØÁÖ°í Àå°Å¸® °ø°ÝÀº ½ÇÁ¦ °ø°Ý ÄÚµå »ðÀÔ 

		CItemElem* pItemElem = g_pPlayer->GetWeaponItem();
		if( pItemElem && pItemElem->GetProp()->dwItemKind3 == IK3_WAND ) 
		{
			// ¸¸¾à ÇÃ·¹ÀÌ¾î°¡ ¿Ïµå·ù¸¦ µé°í ÀÖ¾ú´Ù¸é ¿©±â¼­ Ã³¸®ÇÏÁö ¾Ê°í LBUTTON_UP, ÀÚµ¿°ø°Ý ÀÌ¸ðÆ¼ÄÜ¿¡¼­ Ã³¸®ÇØÁØ´Ù. 
		}
		else
		{
			...
			else
			{
				...
				if( pItemElem && pItemElem->GetProp()->dwItemKind3 == IK3_BOW )
				{
					...
					g_pPlayer->CMD_SetMeleeAttack( pFocusObj->GetId() );		// ±ÙÁ¢ÆÐÅÏ°ø°Ý µé¾î°¨.
			}
		}
	}
	else
	{
		return FALSE;	// ±×¿ÜÀÇ ¿ÀºêÁ§Æ®Å¸ÀÔÀº ÀÏ´Ü ½ÇÆÐ·Î ÇÏÀÚ.
	}
...
BOOL CWndWorld::UseSkillToFocusObj(CCtrl * pFocusObj) {
	...
	if (!g_Option.m_bAutoAttack) return TRUE; // ¿©±â¾ß¿©±â - ÀÚµ¿°ø°Ý ON
...
CObj* CWndWorld::PickObj( POINT point, BOOL bOnlyNPC )
{
	...
	if( pObj )
	{
		...
		else if( pObj->GetType() == OT_MOVER )
		{
			...
			case RANK_BOSS:
			case RANK_MATERIAL:
				// ´ëÇü¸÷ÀÇ °æ¿ì¿£ Á¤¹ÐÇÇÅ·À» ÇÑ¹ø´õ ÇÑ´Ù
...
CObj* CWndWorld::SelectObj( POINT point )
{
	...
	if( pObj )
	{
		...
		{
			// ±æµåÄÄ¹îÁß¿¡´Â Á×ÀºÄÉ¸¯À» ÇÇÅ· ±ÝÁö
			if( pWorld->GetID() == WI_WORLD_GUILDWAR )
			{
				if( ((CMover*)pObj)->IsDie() )
					return NULL;
			}
			if( ((CMover*)pObj)->IsMode( TRANSPARENT_MODE )	) // ´ë»óÀÌ Åõ¸í¸ðµåÀÏ¶© Å¸°Ù ¾ÈµÊ.
			{
				return NULL;
			}			
		}
...
void CWndWorld::SetNextTarget()
{
	...
	if(m_pNextTargetObj && pWorld)
	{
		if( m_pNextTargetObj->GetType() == OT_MOVER )
		{
			...
			if( ((CMover*)m_pNextTargetObj)->IsMode( TRANSPARENT_MODE )	) // ´ë»óÀÌ Åõ¸í¸ðµåÀÏ¶© Å¸°Ù ¾ÈµÊ.
			{
				return;
			}			
		}
...
// ´©ÀûµÈ °ø°Ý°ÔÀÌÁö¸¦ ±¸ÇÑ´Ù. ( °ÔÀÌÁö ´Ü°è:0, 1, 2, 3, 4)
// ¿¹¿Ü:  0  - ¸ðÀ¸Áö ¾ÊÀº »óÅÂ, -1 - 3´Ü°è ¹ß»çÈÄ µô·¹ÀÌ »óÅÂ
...
void CWndWorld::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	{
		if( g_pPlayer->IsAttackAble(pFocusObj) )	// pFocusObj¸¦ °ø°ÝÇÒ¼öÀÖ´ÂÁö °Ë»ç.
		{
...
/*#if __VER >= 8     // 8Â÷ µà¾óÁ¸¿¡ °ü°è¾øÀÌ PVP°¡´ÉÇÏ°ÔÇÔ   Neuz, World
			if( !g_pPlayer->IsBaseJob() && pTarget->IsDie() == FALSE )	// ·¹º§ÀÌ 15ÀÌ»óÀÏ¶§¸¸ ¸Þ´º°¡ ³ªÅ¸³².
			{
				BOOL bInsert = TRUE;
			...
			{
				if( pTarget->IsDie() == FALSE )	// ·¹º§ÀÌ 15ÀÌ»óÀÏ¶§¸¸ ¸Þ´º°¡ ³ªÅ¸³².
				{
					BOOL bInsert = FALSE;
				...
				if( 1 < nCount )
				{
					for( int i = 0; i < MAX_MOVER_MENU; i++ )
					{
						if( i == MMI_QUEST )
						{
							...
							// prop¿¡¼­ ÇØ´ç Ä³¸¯ÅÍ¿¡ ÇØ´çÇÏ´Â Äù½ºÆ®¸¦ ¼öÁýÇÑ´Ù.
							// ¼öÁýÀÌ ¿Ï·áµÇ¸é, ³»±â¼öÇàÇÑ Äù½ºÆ® °¹¼ö¿Í ºñ±³ÇÑ´Ù.
...
void CWndWorld::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if( g_WndMng.GetWndVendorBase() )
		return;
	if( g_pPlayer->m_dwMode & DONMOVE_MODE )	// µ·¹«ºê ¸ðµå¸é ¾Ï°Íµµ ¸øÇÔ.
		return;
//	if( g_pPlayer->GetAdjParam( DST_CHRSTATE ) & CHS_LOOT)			return;
	...
	if( pSelectObj && pSelectObj->GetType() == OT_MOVER )
	{
		...
		if( m_bRButtonDown || m_nMouseMode == 1 )	// ¿ì¹öÆ° ´©¸£°í ÀÖ°Å³ª / ¸¶¿ì½º Ä¿¼­ »ç¶óÁø ¸ðµåÀÏ¶© ¹«Á¶°Ç ¾ÈÀâÈù´Ù.
		{
			m_bAutoAttack = FALSE;
			pSelectObj = NULL;
		}
		else
		{
			...
			else
			{
				...
				{
					m_bAutoAttack = FALSE;
					if(m_pNextTargetObj != NULL) //´ÙÀ½ Å¸°ÙÀÌ ÀÖÀ» °æ¿ì °ø°ÝÇÏ´Â ³ðÀ» ´Ù½Ã ¼±ÅÃÇÏ¸é ´ÙÀ½ Å¸°Ù Á¦°Å
						m_pNextTargetObj = NULL;
					//if( g_Option.m_bAutoAttack )		// ¿©±â¾ß¿©±â - ÀÚµ¿°ø°Ý ON
					//	m_bAutoAttack = TRUE;					  // ÀÚµ¿ °ø°Ý.
					const ItemProp *pItemProp = g_pPlayer->GetActiveHandItemProp();
					if( pItemProp )
					{   
				...
				else if( pSelectOld != NULL && m_bAutoAttack && pSelectOld != pSelectObj ) //ÀÌ¹Ì Å¸°ÙÀÌ ÀâÇô °ø°Ý»óÅÂ¿¡¼­ ´Ù¸¥ Å¸°ÙÀ» ÀâÀ» °æ¿ì
				{
...
//			m_bAutoAttack = TRUE;
	...
	m_dwNextSkill = NEXTSKILL_NONE;	// ¿ÞÂÊ¹öÆ° ´©¸£¸é ½ºÅ³¿¹¾à Ãë¼Ò.

#ifdef __CLIENT
	// ½ºÅ³¹Ù ½ÇÇàÁß¿¡ ¿ÞÂÊ¹öÆ° Å¬¸¯Çß´Ù¸é ½ºÅ³¹Ù ½ÇÇà Ãë¼Ò
	...
	else 
	{
		if( pSelectObj->GetType() == OT_MOVER )
		{
			...
			else
			{
				...
				{
					// 1:1±æµå ´ëÀüÀå ¾È¿¡¼­´Â ÇÃ·¹ÀÌ¾î ¸Þ´º »ç¿ë ºÒ°¡
...
{
	if( m_nMouseMode == 0 )	// ¸ðµå1ÀÏ¶© ¸±¸®Áî ÇØ¼± ¾ÈµÈ´Ù.
		ReleaseCapture();
...
//	m_bAutoAttack = FALSE;
	...
	// ÁÂÃø ¹öÆ°ÀÌ ´­·¯ÁøÁö 1000ºÐÀÇ 200ÃÊ ³»¶ó¸é µ¿½Ã¿¡ ´­·¯Áø °ÍÀ¸·Î ÆÇ´Ü, µ¿½Ã ´­·¯Áø °ÍÀº Á¡ÇÁ, 
...
void CWndWorld::OnRButtonDblClk( UINT nFlags, CPoint point)
{
	...
	if( g_pPlayer->m_pActMover->IsFly() )
	{
		m_nMouseMode ^= 1;		// ºñÇàÁß ¸¶¿ì½º ÀÌµ¿¸ðµå Åä±Û.
		if( m_nMouseMode == 0 )
		{
			ReleaseCapture();
			ClipCursor( NULL ); // À©µµ¿ì¸¦ rcWindow¸¦ ¹þ¾î³ªÁö ¾Ê°Ô ÇÑ´Ù.
		}
		else
		{
			SetCapture();
			...
			ClipCursor( &rcWindow ); // À©µµ¿ì¸¦ rcWindow¸¦ ¹þ¾î³ªÁö ¾Ê°Ô ÇÑ´Ù.
		}
	}
	else
	{
...
void CWndWorld::OnMouseMove(UINT nFlags, CPoint point)
{
...
#endif //__Y_INTERFACE_VER3
	{
		...
		if(g_pPlayer->m_pActMover->IsFly() /*&& m_bMButtonDown == FALSE*/ )
		{
			...
			int nLeft = 0, nTop = 0;
			if( g_Neuz.IsWindow() )	// À©µµ¿ì ¸ðµå·Î ½ÇÇàÁßÀÌ¸é.
			{
				RECT rrr;
...
//		g_Neuz.m_pd3dDevice->ShowCursor( TRUE );
	...
	// ÈÙ¹öÆ°µå·¡±×´Â Ä«¸Þ¶ó¸¸ È¸Àü.
	if( g_pPlayer->m_pActMover->IsFly() )
	{
		if( m_bMButtonDown )	// ºñÇàÁß¿£ ¹«Á¶°Ç ÈÙÀÌ Ä«¸Þ¶óÈ¸ÀüÀÌ´Ù.
			bCamera = TRUE;
	} else
	{	// Áö»ó¿¡¼­¸¸.
		if( g_Option.m_nInterface == 0 )	// ±¸ ¹öÀü.
		{
			if( m_bMButtonDown )		// ÈÙ·Î Ä«¸Þ¶ó¸¦ µ¹¸².
				bCamera = TRUE;
		} 
		else
		if( g_Option.m_nInterface == 1 )	// ±¸ ¹öÀü.
		{		
			if( m_bRButtonDown )		// ¿ìÅ¬¸¯À¸·Î Ä«¸Þ¶ó¸¦ µ¹¸².
				bCamera = TRUE;
		} 
		// ½Å¹öÀü.
#ifdef __Y_INTERFACE_VER3
		...
		if( g_Option.m_nInterface == 2 )
		{		
			if( m_bLButtonDown )		// ÁÂÅ¬¸¯À¸·Î Ä«¸Þ¶ó¸¦ µ¹¸².
			{
				bCamera = TRUE;
			}
...
BOOL CWndWorld::OnDropIcon( LPSHORTCUT pShortcut, CPoint point )
{
	...
	if( pShortcut->m_dwShortcut == ShortcutType::Item ) //&& pWndFrame->GetWndId() == APP_INVENTORY )
	{
		...
		vPos += g_pPlayer->GetPos();			// Àý´ëÁÂÇ¥·Î º¯È¯.
		vPos.y += 1000.0f;		// ÃÖ»óÀ§ ³ôÀÌ¸¦ °Ë»çÇÏ±â À§ÇØ.
		FLOAT fHeight	= pWorld->GetFullHeight( vPos );	// Àý´ëÁÂÇ¥ÀÇ y³ôÀÌ ±¸ÇÔ.
...
//#endif	//
		...
		// Áö»ó¿¡¼­ »ó´ë¸¦ °ø°ÝÇÏ°Å³ª ÇØ´ç ¿ÀºêÁ§Æ® »ç¿ëÇÏ±â.
				...
				if( (m_bLButtonDown || m_bAutoAttack) && (g_pPlayer->GetCmd() != OBJACT_USESKILL) )	// ´©¸¥ ¼ø°£¿¡¸¸ Ã³¸®ÇÏ±â À§ÇØ¼­...
				{
					if( m_bAutoAttack || (m_bSelectTarget && m_timerFocusAttack.IsTimeOut() == TRUE ) )
					{
						if( pObj )
						{
							...
							{	// °Á ÀÏ¹Ý °ø°Ý/»ç¿ë µé¾î°¨.  ½ºÅ³ÀÀ´äÁßÀÏ¶© ½ÇÇàÇÏÁö ¸»ÀÚ.
...
#ifdef __EVE_NEWYEAR
	//	c.12/31ÀÏ 11½Ã59ºÐ ºÎÅÍ ÇÁ¸®ÇÁ ÀÎÅÍÆäÀÌ½º¿¡ °øÁöÃ³·³ ½Ã°£ÀÌ ³ª¿À´Â°Í.
	//	¿¹)23½Ã59ºÐ¶§ °ÔÀÓÃ¢¿¡ 59ÃÊ 58ÃÊ 57ÃÊ 56ÃÊ 55ÃÊ 54ÃÊ........01ÃÊ happy new year
...
#ifdef __YDEBUG
	m_texTarget.RestoreDeviceObjects();		// Áö»ó¿¡¼­ÀÇ 4±ÍÅüÀÌ Å¸°Ù±×¸²
	m_texTargetFly.RestoreDeviceObjects();	// ºñÇàÁß¿¡¼­ÀÇ 4±ÍÅüÀÌ Å¸°Ù±×¸².
	m_texTargetArrow.RestoreDeviceObjects();	// Å¸°ÙÀÌ È­¸éÀ» ¹þ¾î³µÀ»¶§ È­»ìÇ¥¹æÇâÇ¥½Ã.
	m_texGauFlight.RestoreDeviceObjects();	// ºñÇà °ÔÀÌÁö ÀÎÅÍÆäÀÌ½º.

	m_texMsgIcon.RestoreDeviceObjects();	// ºñÇà °ÔÀÌÁö ÀÎÅÍÆäÀÌ½º.
	m_texAttrIcon.RestoreDeviceObjects();	// ºñÇà °ÔÀÌÁö ÀÎÅÍÆäÀÌ½º.
	m_texFontDigital.RestoreDeviceObjects();	// ºñÇà °ÔÀÌÁö ÀÎÅÍÆäÀÌ½º.
#endif //__YDEBUG	
...
//	m_texTarget.InvalidateDeviceObjects();
	...
	m_texTargetArrow.InvalidateDeviceObjects();	// Å¸°ÙÀÌ È­¸éÀ» ¹þ¾î³µÀ»¶§ È­»ìÇ¥¹æÇâÇ¥½Ã.
...
HRESULT CWndWorld::DeleteDeviceObjects()
{
	...
	SAFE_DELETE(m_pWndGuideSystem);
#ifdef __YDEBUG
	m_texTargetArrow.DeleteDeviceObjects();	// Å¸°ÙÀÌ È­¸éÀ» ¹þ¾î³µÀ»¶§ È­»ìÇ¥¹æÇâÇ¥½Ã.
...
//	CObj* pTargetObj = CObj::m_pObjHighlight;		// Ä¿¼­¸¦ ´ë°í ÀÖ´ø ¿ÀºêÁ§Æ®°¡ ÇÏÀÌ¶óÀÌÆ® ¿ÀºêÁ§ÀÌ´Ù.
	...
	if( g_pPlayer->m_pActMover->IsFly() == FALSE )
	{
		if( pTargetObj )	// Ä¿¼­¸¦ ´ë°í ÀÖ´ø ¿ÀºêÁ§Æ®°¡ ÀÖÀ¸¸é
		{
			pWorld->SetObjFocus( pTargetObj );	// ±×³ðÀ» ¼¿·ºÆ® ÇÏ´Â µ¿½Ã¿¡.
...
void CWndWorld::RenderBuffIcon( C2DRender *p2DRender, IBuff* pBuff, BOOL bPlayer, BUFFICON_INFO* pInfo, CPoint ptMouse )
{
	...
	{	
		...
		if( pItem->dwID == II_SYS_SYS_SCR_RETURN )	// ±ÍÈ¯ÀÇ µÎ·ç¸¶¸®´Â ±ôºý°Å¸²
			bFlash	= TRUE;
	}
	...
	{		
		...
		if( pItem->nEvildoing < 0 )							// ³ª»Û¸¶¹ýÀº
			color =  D3DCOLOR_ARGB(alpha, 255, 120, 255 );		// »¡°£ »ö 
		else
			...
			case II_SYS_SYS_SCR_PARTYSUMMON:
				{
					...
					if( ct.GetHours() >= 1 )
						str.Format( "\n%.2d:%.2d:%.2d\n%s", ct.GetHours(), ct.GetMinutes(), ct.GetSeconds(), prj.GetText( TID_GAME_SUMMON_BUFF_ICON ) );	//½ÃºÐÃÊ 
					else
						str.Format( "\n%.2d:%.2d\n%s", ct.GetMinutes(), ct.GetSeconds(), prj.GetText( TID_GAME_SUMMON_BUFF_ICON ) );						// ºÐÃÊ
					RenderOptBuffTime( p2DRender, pInfo->pt, ct, D3DCOLOR_XRGB( 240, 240, 0 ) );
				}
				break;
			...
			default:
				{
					...
					if( pBuff->GetTotal() > 0 )						
					{
						const CTimeSpan ct((long)(dwOddTime / 1000.0f));		// ³²Àº½Ã°£À» ÃÊ´ÜÀ§·Î º¯È¯ÇØ¼­ ³Ñ°ÜÁÜ
...
void CWndWorld::RenderExpBuffIcon( C2DRender *p2DRender, IBuff* pBuff, BUFFICON_INFO* pInfo, CPoint ptMouse, DWORD dwItemID )
{
	...
	for (const auto & [_, ptr] : g_pPlayer->m_buffs.m_mapBuffs) {
		if( dwItemID == II_SYS_SYS_SCR_AMPESE ) // Áßº¹µÇÁö ¾Ê´Â ESÁõÆøÀÇ µÎ·ç¸¶¸® defineitem¿¡¼­ Ã³¸® Àß¸øµÇ¾î µû·Î ºÐ±â
		{
	...
	for( int i = 0 ; i < nExpCount ; ++i )
	{
		...
		CTimeSpan ct( (long)(dwOddTime / 1000.0f) );		// ³²Àº½Ã°£À» ÃÊ´ÜÀ§·Î º¯È¯ÇØ¼­ ³Ñ°ÜÁÜ
		if( ct.GetHours() >= 1 )
			strTime.Format( " %.2d:%.2d:%.2d ", ct.GetHours(), ct.GetMinutes(), ct.GetSeconds() );	//½ÃºÐÃÊ 
		else
			strTime.Format( " %.2d:%.2d ", ct.GetMinutes(), ct.GetSeconds() );						// ºÐÃÊ
...
void CWndWorld::RenderSMBuff( C2DRender *p2DRender, BUFFICON_INFO* pInfo, CPoint ptMouse )
{
	...
	for( int i = 0; i < SM_MAX ; ++i )
	{
		...
		pInfo->pt.x += (32+5);
				...
				if( g_pPlayer->m_dwSMTime[i] < 60 && i != SM_REVIVAL )		// 20ÃÊ ÀÌÇÏ ³²¾ÒÀ¸¸é ±ôºý°Å¸².
				{
...
// ½ºÅ³¹öÇÁ´Â ¸ðµÎ ¿ÞÂÊ¿¡, ¾ÆÀÌÅÛ ¹öÇÁ´Â ¿À¸¥ÂÊ¿¡ Ç¥½ÃÇÑ´Ù.
...
//			|| wType == BUFF_EQUIP
		...
		if( pp != NULL && pp->second.m_pTexture != NULL && bCharged == FALSE )
		{
			...
			{
				Lpoint.x += nIconSize;
				if( pBuff->GetTotal() > 0 && dwOddTime < 20 * 1000 )		// 20ÃÊ ÀÌÇÏ ³²¾ÒÀ¸¸é ±ôºý°Å¸².					
				{
			...
			if( bExpMsg )
			{
				...
				{
					CTimeSpan ct( (long)(dwOddTime / 1000.0f) );		// ³²Àº½Ã°£À» ÃÊ´ÜÀ§·Î º¯È¯ÇØ¼­ ³Ñ°ÜÁÜ
					str.Format( "\n%.2d:%.2d", ct.GetMinutes(), ct.GetSeconds() );		// ³²Àº½Ã°£À» ºÐ/ÃÊ ÇüÅÂ·Î Ãâ·Â.
					strEdit += str;
				}
				else
				{
...
void CWndWorld::ShowCCtrlMenu(const CCommonCtrl * pCCtrl) {
	...
	if( slotIndex < 0 )			//ÇØ´ç ¾ÆÀÌµð¸¦ °¡Áø ³à¼®À» ¸øÃ£À½
		return;
...
void GuildCombatInfo::OnPlayerList(CAr & ar) {
	...
	for (int i = 0; i < nSizeGuild; ++i) {
		u_long uidDefender; ar >> uidDefender;		// µðÆæ´õ
		...
		for (int j = 0; j < nSizeMember; ++j) {
			...
			int nStatus;      ar >> nStatus;		// ÀüÀï »óÅÂ == 1 ; ´ë±âÀÚ == 0
...
void GuildCombatInfo::RenderMyGuildStatus(C2DRender * p2DRender) {
	...
	for (const WndWorld::GuildCombatInfo::GUILDRATE & GuildRate : GuildStatus) {
		...
		if (GuildRate.m_uidPlayer == g_pPlayer->m_idPlayer) {
			...
			// ÁÖÀÎ°øÀÌ°í µé¾î°¥ Â÷·ÊÀÌ¸é¼­ ¶óÀÌÇÁ°¡ ³²¾Æ ÀÖÀ¸¸é ¸Þ¼¼Áö Ãâ·Â
		...
		// ´ÙÀ½Â÷·Ê¿¡ ÃâÀüÇÒ °æ¿ì¶ó¸é ¸Þ¼¼Áö¸¦ ¶ç¿öÁØ´Ù.
